...
//    depthCloud(new pcl::PointCloud<pcl::PointXYZI>) ;
    ...
    cornerPointsSharp.reset(new pcl::PointCloud<pcl::PointXYZI>);
    surfPointsFlat.reset(new pcl::PointCloud<pcl::PointXYZI>);
    cornerPointsLessSharp.reset(new pcl::PointCloud<pcl::PointXYZI>);
    surfPointsLessFlat.reset(new pcl::PointCloud<pcl::PointXYZI>);
    fullPoints.reset(new pcl::PointCloud<pcl::PointXYZI>);
    kdtreeCornerLast.reset(new pcl::KdTreeFLANN<pcl::PointXYZI>());
    kdtreeSurfLast.reset(new pcl::KdTreeFLANN<pcl::PointXYZI>());
    laserCloudCornerLast.reset(new pcl::PointCloud<pcl::PointXYZI>);
    laserCloudSurfLast.reset(new pcl::PointCloud<pcl::PointXYZI>);
    laserCloudFullRes.reset(new pcl::PointCloud<pcl::PointXYZI>);
    lasercornerPointsLessSharp.reset(new pcl::PointCloud<pcl::PointXYZI>);
    lasersurfPointsLessFlat.reset(new pcl::PointCloud<pcl::PointXYZI>);
    q_w_curr.w()=1;q_w_curr.y()=0;q_w_curr.z()=0;q_w_curr.x()=0;
    t_w_curr.x()=0;t_w_curr.y()=0;t_w_curr.z()=0;
    for(int i=0;i<3;i++)para_q[i]=0;
    para_q[3]=1;
    for(int i=0;i<3;i++)para_t[i]=0;
...
//在问题中添加先验信息作为约束
    ...
    pcl::PointCloud<PointType>::Ptr FullPoints(new pcl::PointCloud<PointType>);
    if(!cornerPointsSharpBuf.empty()) {
        CornerPointsSharp->clear();
        CornerPointsSharp = cornerPointsSharpBuf.front();
        cornerPointsSharpBuf.pop();
        SurfPointsFlat->clear();
        SurfPointsFlat = surfPointsFlatBuf.front();
        surfPointsFlatBuf.pop();
        CornerPointsLessSharp->clear();
        CornerPointsLessSharp = cornerPointsLessSharpBuf.front();
        cornerPointsLessSharpBuf.pop();
        SurfPointsLessFlat->clear();
        SurfPointsLessFlat = surfPointsLessFlatBuf.front();
        surfPointsLessFlatBuf.pop();
        FullPoints->clear();
        FullPoints = fullPointsBuf.front();
        fullPointsBuf.pop();
        cornerPointsSharp->clear();
        surfPointsFlat->clear();
        cornerPointsLessSharp->clear();
        surfPointsLessFlat->clear();
        fullPoints->clear();
        *cornerPointsSharp = *tmpcloud + *CornerPointsSharp;
        *surfPointsFlat = *tmpcloud + *SurfPointsFlat;
        *cornerPointsLessSharp = *tmpcloud + *CornerPointsLessSharp;
        *surfPointsLessFlat = *tmpcloud + *SurfPointsLessFlat;
        *fullPoints = *tmpcloud + *FullPoints;
        cout << "get the sum of this frame's cornerPointsSharp is " << cornerPointsSharp->points.size() << endl;
        cout << "get the sum of this frame's surfPointsFlat is " << surfPointsFlat->points.size() << endl;
        cout << "get the sum of this frame's cornerPointsLessSharp is " << cornerPointsLessSharp->points.size() << endl;
        cout << "get the sum of this frame's surfPointsLessFlat is " << surfPointsLessFlat->points.size() << endl;
        cout << "get the sum of this frame's fullPoints is " << fullPoints->points.size() << endl;
    }
    ...
    //下面开始加入激光点的优化
    TicToc t_opt;
//    pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kdtreeCornerLastTmp(new pcl::KdTreeFLANN<pcl::PointXYZI>());
//    pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kdtreeSurfLastTmp(new pcl::KdTreeFLANN<pcl::PointXYZI>());
//    kdtreeCornerLastTmp = kdtreeCornerLast;
//    kdtreeSurfLastTmp = kdtreeSurfLast;
//    if (!cornerPointsSharpBuf.empty())
        if (cornerPointsSharpNum> 0 && surfPointsFlatNum > 0) {
...
//        ceres::Problem problem(problem_options);
                ...
                cout << "判断Kdtree是否为空,1为空，0非空，结果为：" << ((kdtreeCornerLast == nullptr) ? 1 : 0) << endl;
                //待加入临时KDTree变量
...
//        kdtreeSurfLast->setInputCloud(surfPointsLessFlat);
                if (laserCloudCornerLast->points.size() > 0) {
                    for (int i = 0; i < cornerPointsSharpNum; ++i)// i < cornerPointsSharpNum
                    {
                        TransformToStart(&(cornerPointsSharp->points[i]), &pointSel);
                        pointSearchInd.resize(10);
                        pointSearchSqDis.resize(10);

                        int qqq = kdtreeCornerLast->nearestKSearch(pointSel, 1, pointSearchInd, pointSearchSqDis);
                        int closestPointInd = -1, minPointInd2 = -1;
                        if (pointSearchSqDis[0] < 25)//25是DISTANCE_SQ_THRESHOLD,即最远距离阈值
                        {
                            ...
                            for (int j = closestPointInd + 1;
                                 j < (int) laserCloudCornerLast->points.size(); ++j)//scanID方向增大
                            {
                                ...
                                if (int(laserCloudCornerLast->points[j].intensity) >
                                    (closestPointScanID + 2.5))  //2.5是nearby_scan
                                    break;
                ...
                if (laserCloudSurfLast->points.size() > 0) {
                    for (int i = 0; i < surfPointsFlatNum; ++i) {
                        ...
                        if (pointSearchSqDis[0] < 25)//25是DISTANCE_SQ_THRESHOLD
                        {
                            closestPointInd = pointSearchInd[0];

                            // get closest point's scan ID
                            int closestPointScanID = int(laserCloudSurfLast->points[closestPointInd].intensity);
                            double minPointSqDis2 = 25, minPointSqDis3 = 25;  //25是DISTANCE_SQ_THRESHOLD

                            // search in the direction of increasing scan line
                            for (int j = closestPointInd + 1; j < (int) laserCloudSurfLast->points.size(); ++j) {
                                // if not in nearby scans, end the loop
                                if (int(laserCloudSurfLast->points[j].intensity) >
                                    (closestPointScanID + 2.5))//2.5为nearby_scan
                                    break;
...
//t_w_curr和 q_w_curr应该在ceres求解结束后再进行update
//    t_w_curr = t_w_curr + q_w_curr * t_last_curr;   //这里的定义有问题，或许是由于类的原因导致了内存空间不共享t_last_curr并不会同步和para_t进行改变
//    q_w_curr = q_w_curr * q_last_curr;     //这里的定义有问题，或许是由于类的原因导致了内存空间不共享q_last_curr并不会同步和para_q进行改变
    cout<<"before KDTREE cornerPointsLessSharp:"<<cornerPointsLessSharp->points.size()<<endl;
    ...
    cout<<"KDTREE in lasercloudCornerLast:"<<laserCloudCornerLast->points.size()<<endl;
    kdtreeCornerLast.reset(new pcl::KdTreeFLANN<pcl::PointXYZI>());
    kdtreeSurfLast.reset(new pcl::KdTreeFLANN<pcl::PointXYZI>());
...
void Estimator::depthCloudproj(const sensor_msgs::PointCloud2ConstPtr &depthCloud2,const sensor_msgs::PointCloud2ConstPtr& cornerPointsSharpSensor,const sensor_msgs::PointCloud2ConstPtr& surfPointsFlatSensor)
{
...
//	pcl::PointCloud<PointType>::Ptr tmpcloud(new pcl::PointCloud<PointType>);
//	pcl::fromROSMsg(*cornerPointsSharpSensor, *cornerPointsSharp);
//	pcl::fromROSMsg(*surfPointsFlatSensor, *surfPointsFlat);
//	*cornerPointsSharp = *tmpcloud + *tmpcornerPointsSharp;
//	*surfPointsFlat = *tmpcloud + *tmpsurfPointsFlat;
}
...
void Estimator::inputLidarPoints(const pcl::PointCloud<PointType>::Ptr &CornerPointsSharp,const pcl::PointCloud<PointType>::Ptr &SurfPointsFlat,
                                 const pcl::PointCloud<PointType>::Ptr &CornerPointsLessSharp,const pcl::PointCloud<PointType>::Ptr &SurfPointsLessFlat,
                                 const pcl::PointCloud<PointType>::Ptr &FullPoints)
{	
//	pcl::PointCloud<PointType>::Ptr tmpcornerPointsSharp(new pcl::PointCloud<PointType>());
//	pcl::PointCloud<PointType>::Ptr tmpsurfPointsFlat(new pcl::PointCloud<PointType>());
//	pcl::PointCloud<PointType>::Ptr tmpcloud(new pcl::PointCloud<PointType>);
...
//	pcl::fromROSMsg(*CornerPointsSharp, *tmpcornerPointsSharp);
//	pcl::fromROSMsg(*SurfPointsFlat, *tmpsurfPointsFlat);
//	pcl::fromROSMsg(*CornerPointsLessSharp,*tmpcornerPointsLesssharp);
//	pcl::fromROSMsg(*SurfPointsLessFlat,*tmpsurfPointsLessflat);
//	pcl::fromROSMsg(*FullPoints,*tmpfullPoints);
    cornerPointsSharpBuf.push(CornerPointsSharp);
    cornerPointsLessSharpBuf.push(CornerPointsLessSharp);
    surfPointsFlatBuf.push(SurfPointsFlat);
    surfPointsLessFlatBuf.push(SurfPointsLessFlat);
    fullPointsBuf.push(FullPoints);
//	*cornerPointsSharp = *tmpcloud + *CornerPointsSharp;
//	*surfPointsFlat = *tmpcloud + *SurfPointsFlat;
//	*cornerPointsLessSharp = *tmpcloud + *CornerPointsLessSharp;
//	*surfPointsLessFlat = *tmpcloud + *SurfPointsLessFlat;
...
//interpolation ratio插值系数计算
    double s;
    if (DISTORTION)
        s = (pi->intensity - int(pi->intensity)) / 0.1;//0.1 scan_period
    else
        s = 1.0;
    //s = 1;
